...
def emailGateway(email_type,recipients,info):#recipent:[student name, tutor name]
    if(email_type=='session_cancel'):
        ...
        print(email_format.format(" Session has ended (For students)",recipients[0],"The session (ID={}) at {} has ended.\n Your payment will be processed shortly.\n Please use the following link to submit a review: http://127.0.0.1:8000/main/submitReviews/{} \n".format(info.id,localtime(info.session_datetime),info.id)))
        print(email_format.format(" Session has ended (For tutors)",recipients[1],"The session (ID={}) at {} has ended.\n Your transaction will be processed shortly.".format(info.id,localtime(info.session_datetime))))
    elif(email_type=='session_lock'):
        print(email_format.format(" Upcoming Session", recipients[0], "You have a session (ID={}) with {} at {}. You can no longer cancel this session. ".format(info.id,info.session_student,localtime(info.session_datetime))))
        print(email_format.format(" Upcoming Session", recipients[1], "You have a session (ID={}) with {} at {}. They can no longer cancel this session. ".format(info.id,info.session_tutor,localtime(info.session_datetime))))
